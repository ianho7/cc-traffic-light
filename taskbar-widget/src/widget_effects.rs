...
impl WidgetEffectsState {
    pub fn sync_snapshot(&mut self, snapshot: &AppStatusSnapshot, now_ms: u64) {
        self.codex.sync_state(source_state(snapshot, "codex"), now_ms);
        self.claude.sync_state(source_state(snapshot, "claude"), now_ms);
    }

    pub fn render_state_for(&self, snapshot: &AppStatusSnapshot, key: &str, now_ms: u64) -> GroupRenderState {
        ...
        build_group_render_state(state, now_ms)
    }

    pub fn needs_animation_frame(&self, snapshot: &AppStatusSnapshot, now_ms: u64) -> bool {
        [("codex", &self.codex), ("claude", &self.claude)]
            .into_iter()
            .any(|(key, effect)| effect.needs_animation_frame(source_state(snapshot, key), now_ms))
    }
}
...
impl SourceEffectState {
    fn sync_state(&mut self, factual_state: SourceVisualState, now_ms: u64) {
        match factual_state {
            ...
            SourceVisualState::Working | SourceVisualState::NeedsAttention | SourceVisualState::Error => {
...
fn build_group_render_state(display_state: SourceVisualState, now_ms: u64) -> GroupRenderState {
    let lamps = match display_state {
        SourceVisualState::Idle => [lamp(0), lamp(0), lamp(0)],
        SourceVisualState::Working => [lamp(blink_alpha(now_ms, SLOW_BLINK_PERIOD_MS)), lamp(0), lamp(0)],
        SourceVisualState::NeedsAttention => {
            [lamp(0), lamp(blink_alpha(now_ms, FAST_BLINK_PERIOD_MS)), lamp(0)]
        }
        SourceVisualState::Completed => [lamp(ACTIVE_ALPHA), lamp(0), lamp(0)],
        SourceVisualState::Error => [lamp(0), lamp(0), lamp(blink_alpha(now_ms, SLOW_BLINK_PERIOD_MS))],
    };
...
mod tests {
    ...
    fn completed_state_holds_after_snapshot_returns_to_idle() {
        ...
        assert_eq!(
            effects.render_state_for(&idle, "codex", 1_100).display_state,
            SourceVisualState::Completed
        );
        assert_eq!(
            effects.render_state_for(&idle, "codex", 3_000).display_state,
            SourceVisualState::Idle
        );
    }
    ...
    fn blocking_states_cancel_completed_hold_immediately() {
        let mut effects = WidgetEffectsState::default();
        effects.sync_snapshot(&snapshot_with_state("codex", SourceVisualState::Completed), 1_000);
        effects.sync_snapshot(&snapshot_with_state("codex", SourceVisualState::Error), 1_100);

        assert_eq!(
            effects
                .render_state_for(&snapshot_with_state("codex", SourceVisualState::Error), "codex", 1_200)
                .display_state,
    ...
    fn blink_profiles_use_distinct_periods() {
        let working = build_group_render_state(SourceVisualState::Working, 300);
        let attention = build_group_render_state(SourceVisualState::NeedsAttention, 300);
    ...
    fn completed_uses_the_left_green_lamp_position() {
        let completed = build_group_render_state(SourceVisualState::Completed, 300);
